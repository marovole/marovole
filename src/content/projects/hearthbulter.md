...
### Why 76 Database Tables

The database schema is the part I'm most proud of, and also the part that took the longest to get right. Seventy-six tables isn't bloat — it's what you get when you actually model a health data domain properly.
...
Each of these domains has its own internal complexity. The 76 tables reflect that complexity rather than hiding it behind a leaky abstraction. I chose to model the domain correctly rather than shipping something fast that would need to be rebuilt in six months.

### AI Integration Choices

I used OpenAI's API for the reasoning layer, with Upstash Redis for caching repeated nutritional lookups. The AI's job is interpretation and personalization — not data retrieval, which stays in Convex.
...
### Infrastructure: Why OpenNext + Cloudflare Pages

Cloudflare Pages (via OpenNext) for deployment because the target users are families — meaning multiple household members in potentially different locations. Edge distribution matters for perceived latency without cold-start penalties.

Convex for the data layer. Its TypeScript-first schema validation caught modeling errors before they became data bugs. The 76-table schema is defined in `convex/schema.ts`, covering everything from `deviceConnections` and `userConsents` to the Instacart grocery integration. The built-in reactivity handles real-time health data updates without a separate WebSocket layer.

Clerk handles authentication because families need shared-but-separate profiles — one account, multiple health contexts.

Tesseract.js handles OCR for parsing lab report PDFs and images — the primary way health data enters the system. Accurate extraction from varied lab formats turned out to be harder than the AI reasoning layer.
...
The biggest lesson: **complexity that serves the user is different from complexity that serves the engineer.** Seventy-six database tables can be an engineering ego trip or a genuine attempt to model a domain accurately. The test is whether simplifying any part of it would break something real.
...
If I were building this again, I'd spend less time on the AI reasoning layer and more time on the data ingestion layer — getting health data in cleanly is the actual bottleneck, not the intelligence applied to it.
...
| Database | Convex (76 tables, `convex/schema.ts`) |
| Auth | Clerk |
| AI | OpenAI API |
| OCR | Tesseract.js |
